...
# Define features and target (using 'hour' instead of 'time_of_day')
...
def predict_energy_usage():
    while True:
        try:
            ...
            hour = input("Enter hour of the day (0-23) (or type 'exit' to quit): ")
            if hour.lower() == 'exit':
                break
            hour = int(hour)

            # Create DataFrame for input data
            input_data = pd.DataFrame([[temperature, humidity, hour]], columns=['temperature', 'humidity', 'hour'])
